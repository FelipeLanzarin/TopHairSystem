...
public class SubCategoryDao {
	...
	public boolean deleteSubCategory(Integer id) throws ManagersExceptions{
		EntityManager em = EntityManagerUtil.getEntityManager();
		try{
			...
			Throwable t = e;
			while (t != null) {
				if (t.getMessage().contains("ERROR: update or delete on table \"sub_category\" violates foreign key constraint")) {
					ManagersExceptions me = new ManagersExceptions();
					me.setId(ManagersExceptions.CITY_IN_USE);
					me.setExcepetionMessage("Essa SubCategoria já possui um produto. Por esse motivo a mesma não pode ser excluída");
					throw me;
				}
				t = t.getCause();
			}
